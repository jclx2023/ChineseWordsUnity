...
namespace Core.Network
{
    ...
    public class NetworkQuestionManagerController : MonoBehaviourPun
    {
        ...
        private void Awake()
        {
            ...
            {
                Instance = this;
                LogDebug("NetworkQuestionManagerController (修复版) 单例已创建");
            }
            else
            {
                Destroy(gameObject);
        ...
        private void Start()
        {
            ...
            LogDebug("修复版NQMC初始化完成，等待外部启动指令");
        }
        ...
        private void OnDestroy()
        {
            ...
            LogDebug("NetworkQuestionManagerController (修复版) 已销毁");
        }
        ...
        {
            LogDebug($"收到游戏胜利通知: {winnerName} 获胜 - {reason}");
            HandleGameEnded();
        }
        ...
        private void DisableUIInteractions()
        {
            ...
            if (currentManager != null)
            {
                ...
                if (disableMethod != null)
                {
                    ...
                    LogDebug("✓ 题目管理器UI交互已禁用");
                }
                else
                {
                    LogDebug("题目管理器不支持DisableUIInteraction方法");
                }
            }
